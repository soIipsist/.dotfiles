...
case "$LABEL" in
  "Terminal" | "kitty"| "iTerm2")
    ...
    elif grep -q "taskwarrior-tui" <<< "$2"; then RESULT=$ICON_LIST
    elif grep -q "unimatrix\|pipes.sh" <<< "$2"; then RESULT=$ICON_SCREENSAVOR
    elif grep -q "bat" <<< "$2"; then RESULT=$ICON_NOTE
    elif grep -q "tty-clock" <<< "$2"; then RESULT=$ICON_CLOCK
    fi
    ;;
