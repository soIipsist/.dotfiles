...
def clean_file_history(bfg_path:str, git_repository:str, file_path:str):
    ...
    subprocess.run(['java','-jar',bfg_path, '--delete-files', f'{file_path}'], cwd=git_repository)
    
    # git reflog
    subprocess.run(split("git reflog expire --expire=now --all && git gc --prune=now --aggressive"), cwd=git_repository)
...
if __name__ == "__main__":
    ...
    parser.add_argument('git_repository',type=is_git_repository, help="Git repository from which to erase file history.")
    parser.add_argument('-f','--file_path', type=is_valid_path, help="File path in git repository.")
    parser.add_argument('-b','--bfg_directory', default=os.getcwd(), type=is_valid_dir, help="bfg.jar path. If not specified, base directory will be used by default.")
    parser.add_argument('-v','--bfg_version', default="1.14.0", help="Downloads the specified version of bfg if the relevant bfg.jar file is not already present.")
